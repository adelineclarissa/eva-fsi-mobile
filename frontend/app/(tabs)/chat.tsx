...
import { sendChatMessage, streamChatMessage, uploadDocument } from '../../services/apiService';
...
export default function ChatScreen() {
  ...
  const { refresh: refreshAccounts } = useAccountStore();
  ...
  const handleUploadPDF = useCallback(async () => {
    ...
    try {
      ...
      const uploadResult = await uploadDocument(file.uri, file.name, file.mimeType || 'application/pdf');
  ...
  const handleClearChat = useCallback(() => {
    Alert.alert(
      ...
      [
        ...
        { text: 'Clear', style: 'destructive', onPress: () => { clearChat(); Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Heavy); } },
      ]
    );
  }, [clearChat]);
      ...
      {messages.length === 0 && (
        <Animated.View entering={FadeIn.delay(300)} style={styles.welcomeContainer}>
          ...
          <View style={styles.capabilitiesGrid}>
            {[
              { icon: 'wallet-outline', label: 'Cek Saldo', color: Colors.accentTeal },
              { icon: 'send-outline', label: 'Transfer Dana', color: Colors.accentPurple },
              { icon: 'swap-horizontal-outline', label: 'Cek Kurs', color: Colors.accentGold },
              { icon: 'document-text-outline', label: 'Info Produk Deposito', color: Colors.accentRose },
            ].map((cap) => (
              <View key={cap.label} style={styles.capabilityChip}>
                <Ionicons name={cap.icon as any} size={16} color={cap.color} />
                <Text style={[styles.capabilityLabel, { color: cap.color }]}>{cap.label}</Text>
              </View>
            ))}
          </View>
        </Animated.View>
    ...
    <View>
      ...
      {isStreaming && streamingContent && (
        <View style={styles.streamingContainer}>
          <LinearGradient colors={[Colors.accentPurple, Colors.accentTeal]} style={styles.streamAvatar}>
  ...
  return (
    <SafeAreaView style={styles.safe} edges={['top']}>
      ...
      >
        ...
        {/* Input area */}
        <Animated.View entering={SlideInUp.springify()} style={styles.inputContainer}>
          <View style={styles.inputRow}>
            {/* Upload PDF button */}
            <Pressable
              onPress={handleUploadPDF}
              style={[styles.inputAction, isUploading && styles.inputActionDisabled]}
          ...
          <Text style={styles.disclaimer}>Powered by Claude · Responses may be inaccurate</Text>
        </Animated.View>
      </KeyboardAvoidingView>
    </SafeAreaView>
  );
