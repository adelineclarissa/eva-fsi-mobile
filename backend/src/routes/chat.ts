...
router.post("/", async (req: Request, res: Response) => {
  try {
    const { message, sessionId, activeAccountId } = req.body;

    console.log("=== Chat Request (non-stream) ===");
    ...
    if (activeAccountId) {
      setActiveAccount(sid, activeAccountId);
    }
...
router.post("/stream", async (req: Request, res: Response) => {
  ...
  console.log("=== Chat Request (stream) ===");
  console.log("message:", message);
  console.log("sessionId:", sessionId);
  console.log("activeAccountId:", activeAccountId);
  console.log("full body:", JSON.stringify(req.body, null, 2));
  ...
  try {
    await streamChat(
      ...
      (accounts) => {
        sendEvent("accountSelect", { needsSelection: true, accounts });
        res.end();
      },
    );
  } catch (error) {
